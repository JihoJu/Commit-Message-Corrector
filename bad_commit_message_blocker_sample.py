...
def check_subject_is_not_too_long(commit_message, subject_limit):
    ...
    print_result(check_result, "Limit the subject line to " + str(subject_limit) + " characters")
...
def check_subject_is_capitalized(commit_message):
    ...
    # if check_type_is_specified(commit_message):
    #   check_result = not lines[0][0].isupper()
    # else:
    #   check_result = lines[0][0].isupper()
...
commit_type_list = [ "feat:", "fix:", "design:", "build:", "chore:","ci:", "docs:", "style:","refactor:", "test:","rename:", "remove:"]

def check_type_is_specified(commit_message):
    regex = "\(.*\)|\s-\s.*"
    commit_message = re.sub(regex,'', str(commit_message))
    commit_message = commit_message.split() 

    if commit_message[0] not in commit_type_list:
      check_result = False
    else:
      check_result = True

    print_result(check_result, "Use the correct type keyword")
...
def check(commit_message, subject_limit=DEFAULT_SUBJECT_LIMIT, body_limit=DEFAULT_BODY_LIMIT):
    ...
    all_rules_verified &= check_type_is_specified(commit_message)
